...
class Config:
    ...
    TOP_P = float(os.getenv('TOP_P', '0.7'))  # Focused sampling
    REPEAT_PENALTY = float(os.getenv('REPEAT_PENALTY', '1.1'))  # Reduce repetition
    SEED = int(os.getenv('SEED', '42'))  # Fixed seed for deterministic results fir fast reply
    ...
    # Prompt Configuration
    PROMPT_TEMPLATE_WITH_SPECS111 = """
# Role
You are a professional product content writer for a technical B2B website. Your task is to generate consistent, SEO-optimized, and uniquely detailed content for industrial components and equipment using the provided part number, manufacturer, and available specifications. Your output must be accurate, detailed, and targeted to electrical contractors, resellers, and end users.
...
1. PRIORITY: Use the provided product specifications as the primary source of truth. Only infer details when specifications are missing or incomplete, and ensure they are reasonable based on typical products from this manufacturer.
2. DO NOT invent features or specifications not supported by either the provided data or a reasonable inference based on part number/manufacturer patterns.
3. Make every title and description unique and optimized for SEO — never reuse text across part numbers.
4. Include as many of the provided specifications as possible in the description: amperage, voltage, AIC ratings, enclosure type, pole count, mounting type, NEMA/IP rating, certifications, etc.
5. If any key technical detail is missing, infer it carefully from part number patterns or manufacturer standards. Clearly state inferred specifications as typical or standard if necessary.
6. Deliver **maximum technical depth** in every description: electrical parameters, environmental ratings, material details, safety certifications, mechanical specifications, installation requirements, maintenance considerations, and performance characteristics.
7. Present the information in a highly professional tone, using precise technical terminology, industry jargon, and detail suitable for engineers, contractors, and procurement specialists.
8. Include numerical values where available (or where reasonable to infer): amperage, voltage, dimensions, torque, temperature ranges, wire gauge ratings, power ratings, efficiency, IP/NEMA ratings, etc.
9. Avoid generic marketing fluff — focus on technical accuracy, compliance, and performance benefits.
10. The result should sound like a manufacturer’s catalog description written by a technical content specialist.

# Output Format
CRITICAL: Start your response directly with the title and description. Do not include any introductory text, notes, or explanations.

Title: [SEO Title]  
Description: [Technical Product Description]
...
Part of the Square D PowerPact H-Series, the HDA36100 features a thermal-magnetic trip unit, durable molded case, and bolt-on connections that simplify panel installation. Its compact design makes it a perfect fit for new installations, replacements, or retrofits in high-demand environments.
Electricians, contractors, and facility managers rely on Square D for quality—and at Essential Parts, we deliver the products you need fast. All breakers are in stock, tested, and backed by expert support.
Buy the HDA36100 today and keep your power systems protected and code-compliant.Key Specifications Section** (bullet-point list of main technical parameters from the provided specs and any reasonable inferences)
